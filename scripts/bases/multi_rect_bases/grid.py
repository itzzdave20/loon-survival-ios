...
import scripts.modules.graph.coord_converter
coord_cnvrtr = scripts.modules.graph.coord_converter
import scripts.bases.multi_rect_bases.multi_rect_base
MultiRectBase = scripts.bases.multi_rect_bases.multi_rect_base.MultiRectBase
...
class Grid(MultiRectBase):
	...
	def set_block_colour(self, rect, idx, use_texture=False):
		...
		idx_world = int(wx) + int(wy) * self.game.graph_mngr.world_width
		# guard against out-of-range indices from coordinate rounding
		if idx_world < 0 or idx_world >= len(self.game.graph_mngr.world.string):
			w_string = ' '
		else:
			w_string = self.game.graph_mngr.get_string(idx_world)
