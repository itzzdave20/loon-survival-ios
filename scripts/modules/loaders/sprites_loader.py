...
import os
import csv
import scripts.screen_objects.sprites as sprites
globals().update({k: v for k, v in sprites.__dict__.items() if not k.startswith('__')})
...
def init(game_ref, load_list):
	...
	# If the current world has embedded spawn markers, append them to the load list
	try:
		world = _game_ref.graph_mngr.world
		if hasattr(world, 'spawn_list') and world.spawn_list:
			for entry in world.spawn_list:
				# entry already in form (type, id, (x,y))
