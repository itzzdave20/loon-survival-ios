...
import scripts.cards.card
Card = scripts.cards.card.Card
import scripts.bases.window_objects.drop_down_base
DropDownBase = scripts.bases.window_objects.drop_down_base.DropDownBase
...
_tkwargs = {'text':'Loon Survival', 'font_name':'AdventureRequest', 'halign':'center', 'valign':'top', 'font_size':140,  'text_color':(1,1,1,1), 'text_size':(0,0), 'padding_x':20, 'padding_y':0}
...
class MenuCard(Card):
	...
	def re_init(self):
		...
		size = 700, 100
		pos = self.window.width / 2, self.window.height / 6
		self.dp_down_comp.re_init(size=size, pos=pos)
